...
import maui.stemmers.SremovalStemmer;
...
public class MauiWrapper {
		...
		for (int i = 0; i < topicsPerDocument; i++) {
			if (topRankedInstances[i] != null) {
				String topic = topRankedInstances[i].stringValue(extractionModel
						.getOutputFormIndex());
				topics.add(topic + "#" + topRankedInstances[i].stringValue(0));
			}
		}
	...
	public static void main(String[] args) {
		...
		*/
		...
		// Extracting topics without a vocabulary
		// (keyphrase extraction)
		 String vocabularyName = "none";
		String modelName = "cul180";
		String dataDirectory = "../Maui1.2/";
			
		MauiWrapper wrapper = new MauiWrapper(dataDirectory, "none", modelName);
		...
		try {
			...
			ArrayList<String> keywords = wrapper.extractTopicsFromFile(filePath, 500);
